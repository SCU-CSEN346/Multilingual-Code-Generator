...
def compile_worker(dir_list_chunk):
    ...
    with (
        ...
        for folder in tqdm(
            ...
            try:
                temp_path = f"{out_path}/template.py"
                pre_path = f"{root_path}/{folder}/source.py"
                src_path = f"{root_path}/{folder}/source_codonified.py"
                ...
                with(
                    ...
                    src_file.write(f"{temp_file.read()}\n{pre_file.read()}")
                ...
                print(f"Worker - {current_process().pid}: {e}")
                continue
